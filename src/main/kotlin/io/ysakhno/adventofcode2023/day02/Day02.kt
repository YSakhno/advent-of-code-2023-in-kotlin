...
private data class Game(val id: Int, val cubeSets: List<CubeSet>) {
    val isPossible
        get() = cubeSets.all { it.red <= 12 && it.green <= 13 && it.blue <= 14 }
}

private fun Game.findMinimumSetOfCubesNecessary() = cubeSets.reduce { set1, set2 ->
    CubeSet(max(set1.red, set2.red), max(set1.green, set2.green), max(set1.blue, set2.blue))
}
...
private fun String.parseCubeSet(): CubeSet {
    ...
    var countGreen = 0
    var countBlue = 0
    for (cube in splitTrimming(',')) {
        when {
            cube.endsWith(" red") -> countRed += cube.removeSuffix(" red").toInt()
            cube.endsWith(" green") -> countGreen += cube.removeSuffix(" green").toInt()
            cube.endsWith(" blue") -> countBlue += cube.removeSuffix(" blue").toInt()
        }
    }
    return CubeSet(countRed, countGreen, countBlue)
}

private fun String.parseGame(): Game {
    val (idStr, setsStr) = splitTrimming(':')
    val id = idStr.removePrefix("Game ").toInt()
    val sets = setsStr.splitTrimming(';').map { it.parseCubeSet() }
