 ...
 * from the point where light enters the facility. The boxes have holes that allow light to pass from one box to the
 ...
 *   lenses already in the box. Don't move any of the other lenses when you do this. If there aren't any lenses in the
 ...
 *
 * To confirm that all lenses are installed correctly, add up the focusing power of all the lenses. The focusing power
 ...
 */
package io.ysakhno.adventofcode2023.day15
...
private val String.hash get() = fold(0) { hash, ch -> (hash + ch.code) * 17 % 256 }

private data class Lens(val label: String, val focalLength: Int) {
    override fun toString() = "[$label $focalLength]"
}

private fun part1(input: List<String>) = input.flatMap { it.split(',') }.sumOf(String::hash)

private fun part2(input: List<String>) =
    input.flatMap { it.split(',') }
        .fold(List(256) { mutableListOf<Lens>() }) { boxes, step ->
            if (step.contains('-')) {
                ...
                box.indexOfFirst { it.label == label }.also { idx -> if (idx == -1) box.add(lens) else box[idx] = lens }
            }
            boxes
        }
        .flatMapIndexed { boxIdx, lenses ->
            lenses.mapIndexed { lensIdx, lens -> (boxIdx + 1) * (lensIdx + 1) * lens.focalLength }
        }
        .sum()
