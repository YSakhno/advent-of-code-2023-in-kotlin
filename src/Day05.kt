 ...
 *
 * The almanac lists the seeds that need to be planted. It also lists what type of soil to use with each kind of seed,
 * what type of fertilizer to use with each kind of soil, what type of water to use with each kind of fertilizer, and so
 * on. Every type of seed, soil, fertilizer and so on is identified with a number, but numbers are reused by each
 ...
 * corresponds to a destination range starting at 52 and also containing 48 values: 52, 53, ..., 98, 99. So, seed number
...
private data class RangedMapping(val dest: Long, val src: Long, val length: Long): Comparable<RangedMapping> {
    ...
    operator fun contains(index: Long) = index >= src && index < src + length
    operator fun get(index: Long) = if (index in this) dest + (index - src) else index
}
